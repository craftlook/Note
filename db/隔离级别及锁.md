...
1. 事务一
   ...
   - WAITING FOR THIS LOCK TO BE GRANTED …… lock_mode X locks gap before rec **insert intention waiting** 等待insert intention lock
   
2. 事务二
   
   - HOLDS THE LOCK(S) …… lock mode S locks gap before rec Record lock 
   
   - WAITING FOR THIS LOCK TO BE GRANTED …… lock_mode X locks gap before rec **insert intention waiting** 等待insert intention lock

**2.补充Insert中对唯一索引的加索逻辑：**

1. 插入时先对唯一索引（unique key 下文简称**UK**），先对目标行加[S](#S)锁中的[N](#N)锁（**S** lock中的next key lock 下文简称**S**、**N**），这个锁如果最终插入成功（该记录在等待期间被其他事务删除，次所被同事删除）；
2. 如果条件1成功，对对应得行增加[X](#X)锁中的[I](#I)锁；
3. 如果条件2成功，插入记录，并对记录增加[X](#X)锁中的[R](#R)锁（有可能是<a href="http://mysql.taobao.org/monthly/2020/09/06/">隐式锁</a>）

   **总结Insert加索顺序:** [S](#S)中的[N](#N)锁 -> [X](#X)中的[I](#I)锁 -> [X](#X)中的[R](#R)锁

**3. 锁细节分析**

1. [扩展：RC 与 RR 的区别](#RC_RR_DIFF)
...
![avatar](https://github.com/craftlook/Hello-World/blob/craftlook-Hello-World/image/next-key.png)

除了Next-Key Locking，还有Previous-Key Locking技术。同样上述的值，使用Previous-Key Locking技术，那么可锁定的区间为：
![avatar](https://github.com/craftlook/Hello-World/blob/craftlook-Hello-World/image/previous-key.png)
