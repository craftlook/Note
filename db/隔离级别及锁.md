...
1. 事务一
   ...
   - WAITING FOR THIS LOCK TO BE GRANTED …… lock_mode X locks gap before rec **insert intention waiting** Record lock 等待插入意向排他锁
...
1. 插入时先对唯一索引（unique key 下文简称**UK**）进行冲突检测，先对目标行加[S](#SK)锁中的[N](#N)锁（**S** lock中的next key lock 下文简称**S**、**N**），这个锁如果最终插入成功（该记录在等待期间被其他事务删除，次所被同事删除）；
2. 如果条件1成功，对应的行增加[X](#X)锁中的[I](#I)锁；
